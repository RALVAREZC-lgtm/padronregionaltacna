...
# ==============================================================================
# 4. CARGA DE DATOS UNIFICADA (TACNA, TARATA, CANDARAVE, JORGE BASADRE)
# ==============================================================================
@st.cache_data
def cargar_padron_unificado() -> pd.DataFrame:
    ...
    for archivo, prov_default in archivos:
        try:
            ...
            if "NOMPRO" not in df_temp.columns:
...
# ==============================================================================
# 6. NAVEGACIÓN Y MENÚ SEGÚN ROLES
# ==============================================================================
...
opciones_menu = []
if user["rol"] in ["Administrador", "Analista", "Operador"]:
    opciones_menu.append("🪪 Búsqueda & Fichas")
if user["rol"] in ["Administrador", "Analista"]:
    opciones_menu.append("📊 Dashboard Estadístico")
    opciones_menu.append("🗺️ Geolocalización / Mapas")
    opciones_menu.append("📥 Exportar Reportes")
...
st.markdown(
    ...
    <div class="app-header">
        ...
        <p>Sistema Unificado de Consulta, Análisis Geoespacial e Interoperabilidad</p>
    </div>
""",
    unsafe_allow_html=True,
)
...
try:
    ...
    # MÓDULO 3: GEOLOCALIZACIÓN Y MAPAS (FOLIUM)
        ...
        st.subheader("🗺️ Ubicación de Locales de Votación de Tacna")
    ...
    # MÓDULO 5: GESTIÓN DE USUARIOS
    elif seccion_activa == "⚙️ Gestión de Usuarios":
        st.subheader("⚙️ Panel de Control de Usuarios y Perfiles (RBAC)")

        tab_list, tab_add = st.tabs(["📋 Usuarios Registrados", "➕ Registrar Nuevo Usuario"])

        with tab_list:
            users_data = [{"Usuario": u, "Nombre": d["nombre"], "Rol": d["rol"]} for u, d in st.session_state.db_usuarios.items()]
            st.dataframe(pd.DataFrame(users_data), use_container_width=True)

            st.divider()
            st.markdown("#### Editar Usuario")
            c_u, c_p, c_r = st.columns(3)
            with c_u:
                usr_mod = st.selectbox("Usuario", list(st.session_state.db_usuarios.keys()))
            with c_p:
                pass_mod = st.text_input("Nueva Contraseña", type="password")
            with c_r:
                rol_mod = st.selectbox("Rol", ["Administrador", "Analista", "Operador"])
        ...
        with tab_add:
            with st.form("form_add"):
                ...
                r_new = st.selectbox("Rol", ["Administrador", "Analista", "Operador"])
                if st.form_submit_button("Crear Usuario"):
                    ...
                    if u_clean and p_new.strip() and u_clean not in st.session_state.db_usuarios:
                        ...
                        st.success(f"Usuario {u_clean} creado.")
                        time.sleep(0.5)
                        st.rerun()
